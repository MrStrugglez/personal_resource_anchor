...
function lib:SetFrameSetting(frame, setting, value)
  ...
  if restrictions then
    ...
    assert(min <= value and value <= max,
      string.format("Value %s invalid for this setting: min %s, max %s", value, min, max))
  end
...
function lib:ReanchorFrame(frame, ...)
  ...
  anchorInfo.point, anchorInfo.relativeTo, anchorInfo.relativePoint, anchorInfo.offsetX, anchorInfo.offsetY = pointGetter
      :GetPoint(1)
